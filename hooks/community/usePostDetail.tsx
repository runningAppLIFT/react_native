...
const fetchPost = useCallback(async () => {
  ...
  try {
    ...
    // 서버가 직접 PostDetail 객체를 반환하는 경우 처리
    if ('comm_number' in result) {
      setPost(result as PostDetail);
    } else if (result.post) {
      setPost(result.post);
    } else {
      console.warn('API returned no post data for postId:', postId);
  ...
  const updatePost = useCallback(
    async (data: { comm_title: string; comm_detail: string }) => {
      if (!postId) {
